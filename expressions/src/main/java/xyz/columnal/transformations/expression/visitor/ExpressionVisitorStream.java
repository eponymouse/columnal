...
public class ExpressionVisitorStream<T> implements ExpressionVisitor<Stream<T>>
{
    ...
    @Override
    public Stream<T> ident(@Recorded IdentExpression self, @Nullable @ExpressionIdentifier String namespace, ImmutableList<@ExpressionIdentifier String> idents, boolean isVariable)
    {
        return Stream.of();
    }
    ...
    @Override
    public Stream<T> define(DefineExpression self, ImmutableList<DefineItem> defines, @Recorded Expression body)
    {
        return Stream.<T>concat(defines.stream().<T>flatMap(e -> e.typeOrDefinition.<Stream<T>>either(x -> x.visit(this), x -> Stream.<T>concat(x.lhsPattern.<Stream<T>>visit(this), x.rhsValue.<Stream<T>>visit(this)))), body.<Stream<T>>visit(this));
    }

    @Override
    public Stream<T> hasType(@Recorded HasTypeExpression self, @ExpressionIdentifier String lhsVar, @Recorded Expression rhsType)
    {
        return apply(rhsType);
    }
    ...
    @Override
    public Stream<T> field(FieldAccessExpression self, @Recorded Expression lhsRecord, String fieldName)
    {
        return lhsRecord.<Stream<T>>visit(this);
    }
